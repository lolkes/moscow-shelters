...
async function loadShelters(){try{const r=$('#region').value;const res=await fetch('/api/shelters'+(r?'?region='+encodeURIComponent(r):''));const data=await res.json();if(!res.ok||!Array.isArray(data))throw new Error(data?.error||'shelters api');$('#shelterGrid').innerHTML=data.slice(0,24).map(s=>'<article class="shelter-card"><div class="shelter-head"><span>⌂</span><i class="'+(s.verified?'verified':'')+'">'+(s.verified?'Проверен':'Источник')+'</i></div><h3>'+esc(s.name)+'</h3><p>'+esc(s.city||s.region||'Москва и МО')+'</p><strong>🐾 '+(s.animal_count||0)+' животных</strong><div class="shelter-actions"><a href="/shelters/'+encodeURIComponent(s.id)+'">Каталог</a>'+(s.website?'<a target="_blank" rel="noopener" href="'+esc(s.website)+'">Сайт ↗</a>':'')+'</div></article>').join('')}catch(e){console.error(e);$('#shelterGrid').innerHTML='<div class="empty">Не удалось загрузить список приютов. Обновите страницу.</div>'}}
